//! Core bundler implementation (no Magnus dependencies)
...
/// Core bundler (no Magnus dependencies)
#[derive(Clone)]
...
impl CoreBundler {
    ...
    pub async fn bundle(&self) -> Result<fob_bundler::BuildResult, fob_bundler::Error> {
        ...
        let out_dir = if let Some(output_dir) = &self.config.output_dir {
            ...
            // Simple validation - check if path is within cwd
            if path.is_absolute() && !path.starts_with(&cwd) {
                return Err(fob_bundler::Error::InvalidOutputPath(
                    "Output directory must be within working directory".to_string(),
                ));
            }
            cwd.join(path)
        } else {
        ...
        for entry in &self.config.entries {
            ...
            if entry_path.is_absolute() && !entry_path.starts_with(&cwd) {
                return Err(fob_bundler::Error::InvalidConfig(format!(
                    "Entry path must be within working directory: {}",
                    entry
                )));
            }
        }
