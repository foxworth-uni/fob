...
use fob_store::{GraphQueries, GraphStorage};
...
pub struct ModuleGraph {
    storage: Arc<fob_store::GraphStorage>,
}
...
impl ModuleGraph {
    ...
    ///
    /// Requires `fob-store` dependency and `storage` feature.
    #[cfg(feature = "storage")]
    ...
    pub async fn add_module(&self, module: Module) -> Result<()> {
        self.storage
            .store_module(&module)
            .await
            .map_err(|e| Error::InvalidConfig(format!("Failed to store module: {e}")))?;
        ...
        if module.is_entry {
            ...
            self.storage
                ...
                .await
                .map_err(|e| Error::InvalidConfig(format!("Failed to update entry point: {e}")))?;
        }
    ...
    pub async fn add_dependency(&self, from: ModuleId, to: ModuleId) -> Result<()> {
        self.storage
            ...
            .await
            .map_err(|e| Error::InvalidConfig(format!("Failed to add dependency: {e}")))?;
        Ok(())
    }
    ...
    pub async fn module(&self, id: &ModuleId) -> Result<Option<Module>> {
        self.storage
            .get_module(id)
            .await
            .map_err(|e| Error::InvalidConfig(format!("Failed to get module: {e}")))
    }
    ...
    pub async fn modules(&self) -> Result<Vec<Module>> {
        self.storage
            .get_all_modules()
            .await
            .map_err(|e| Error::InvalidConfig(format!("Failed to get modules: {e}")))
    }
    ...
    pub async fn dependencies(&self, id: &ModuleId) -> Result<Vec<ModuleId>> {
        self.storage
            .get_dependencies(id)
            .await
            .map_err(|e| Error::InvalidConfig(format!("Failed to get dependencies: {e}")))
    }
    ...
    pub async fn dependents(&self, id: &ModuleId) -> Result<Vec<ModuleId>> {
        self.storage
            .get_dependents(id)
            .await
            .map_err(|e| Error::InvalidConfig(format!("Failed to get dependents: {e}")))
    }
    ...
    pub async fn entry_points(&self) -> Result<Vec<ModuleId>> {
        self.storage
            .get_entry_points()
            .await
            .map_err(|e| Error::InvalidConfig(format!("Failed to get entry points: {e}")))
    }
    ...
    pub async fn external_dependencies(&self) -> Result<Vec<ExternalDependency>> {
        self.storage
            ...
            .await
            .map_err(|e| Error::InvalidConfig(format!("Failed to get external dependencies: {e}")))
    }
    ...
    /// # async fn example(graph: &ModuleGraph) -> fob::Result<()> {
    ...
    ///
    /// ```no_run
    /// # use fob::graph::{ModuleGraph, PackageJson};
    /// # async fn example(graph: &ModuleGraph, pkg: &PackageJson) -> fob::Result<()> {
    ...
    ///
    /// ```no_run
    /// # use fob::graph::{ModuleGraph, ModuleId};
    /// # async fn example(graph: &ModuleGraph, target: &ModuleId) -> fob::Result<()> {
