...
pub fn create_deep_project(temp: &TempDir, depth: usize) -> PathBuf {
    ...
    let mut owned_files: Vec<(String, String)> = files;
...
pub async fn assert_graph_contains_module(
    graph: &crate::graph::ModuleGraph,
    path: &str
) -> bool {
    let modules = graph.modules().await.unwrap();
    modules.iter().any(|m| m.path.to_string_lossy().contains(path))
}

/// Assert that analysis found a circular dependency.
pub async fn assert_has_circular_dependency(
