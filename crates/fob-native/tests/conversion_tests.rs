...
async fn test_bundle_result_conversion_preserves_structure() {
    ...
    assert!(!napi_result.chunks[0].code.is_empty(), "Chunk should have code");
    ...
    for chunk in &napi_result.chunks {
        for module in &chunk.modules {
            ...
            assert!(module.size.is_none(), "size should be None until implemented");
            assert!(module.has_side_effects.is_none(), "has_side_effects should be None until implemented");
