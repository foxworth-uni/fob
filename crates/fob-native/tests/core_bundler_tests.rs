...
async fn test_core_bundler_without_napi() {
    ...
    match &result {
        ...
        Err(e) => {
            panic!("CoreBundler should bundle successfully, but got error: {:?}", e);
        }
    }
}
...
async fn test_single_entry_bundle() {
    ...
    assert!(!output.assets.is_empty(), "Should have at least one output asset");
}
...
async fn test_multiple_entry_bundle() {
    ...
    let runtime = Arc::new(NativeRuntime::new(cwd.clone()).unwrap());
    let result = BuildOptions::components(vec![
        cwd.join("a.js"),
        cwd.join("b.js"),
    ])
    .cwd(cwd)
...
async fn test_bundle_with_sourcemap() {
    ...
    let output = build_result.output.as_single().unwrap();
    let has_sourcemap = output.assets.iter().any(|asset| {
        asset.filename().ends_with(".map")
    });
...
async fn test_bundle_different_formats() {
    ...
    for format in formats {
        ...
        assert!(result.is_ok(), "Build should succeed for format {:?}", format);
    }
}
