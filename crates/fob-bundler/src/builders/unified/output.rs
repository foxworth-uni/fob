...
pub struct BuildResult {
    ...
    pub analysis: fob_graph::AnalysisResult,

    /// Cache effectiveness metrics.
    pub cache: fob_graph::CacheAnalysis,

    /// Transformation trace (when JOY_TRACE env var is set).
    pub trace: Option<fob_graph::TransformationTrace>,
...
impl BuildResult {
    ...
    pub fn analysis(&self) -> &fob_graph::AnalysisResult {
        &self.analysis
    }
    ...
    pub fn cache(&self) -> &fob_graph::CacheAnalysis {
        &self.cache
    }

    /// Access transformation trace (if enabled via JOY_TRACE).
    pub fn trace(&self) -> Option<&fob_graph::TransformationTrace> {
        self.trace.as_ref()
    }
...
impl BuildOutput {
    ...
    pub fn write_to(&self, dir: impl AsRef<Path>, overwrite: bool) -> Result<()> {
        ...
        eprintln!(
            "[DEBUG] BuildOutput::write_to called, dir: {}",
            dir.as_ref().display()
        );
        match self {
            BuildOutput::Single(bundle) => {
                eprintln!(
                    "[DEBUG] Single bundle mode, assets: {}",
                    bundle.assets.len()
                );
            ...
            BuildOutput::Multiple(bundles) => {
                ...
                for (name, bundle) in bundles {
                    eprintln!(
                        "[DEBUG] Writing bundle '{}' with {} assets",
                        name,
                        bundle.assets.len()
                    );
