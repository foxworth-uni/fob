...
fn test_dependency_dependent_symmetry() {
    ...
    for (from_path, to_path) in test_cases {
        ...
        if let (Some(from), Some(to)) = (from_id, to_id) {
            ...
            let _ = graph.add_module(Module::builder(
                from.clone(),
            ...
            if graph.add_dependency(from.clone(), to.clone()).is_ok() {
                ...
                assert!(dependents.contains(&from), "Dependency should create dependent relationship");
            }
        }
    }
}
...
fn test_no_dangling_edges() {
    ...
    assert!(result.is_ok(), "add_dependency doesn't validate module existence");
    
    // Verify dependency was added
    let deps = graph.dependencies(&id1).expect("Should query dependencies");
    assert!(deps.contains(&id2), "Dependency should be added even if target doesn't exist");
}
