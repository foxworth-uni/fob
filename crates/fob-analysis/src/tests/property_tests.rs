...
use crate::config::{AnalyzerConfig, DEFAULT_MAX_DEPTH, DEFAULT_MAX_MODULES};
...
fn test_config_defaults_reasonable() {
    ...
    assert!(DEFAULT_MAX_DEPTH <= 10000, "max_depth should be at most 10000");
    
    assert!(DEFAULT_MAX_MODULES >= 1000, "max_modules should be at least 1000");
    assert!(DEFAULT_MAX_MODULES <= 1_000_000, "max_modules should be at most 1M");
}

/// Property test: path normalization invariants.
#[cfg(feature = "proptest")]
#[proptest]
fn test_path_normalization_invariants(
    #[strategy("[a-zA-Z0-9_./-]{0, 50}")] path: String,
) {
    use std::path::Path;
    ...
    assert!(dot_dot_count <= 10, "Path should not have excessive .. components");
}

/// Property test: resolver handles random import specifiers without panicking.
#[cfg(feature = "proptest")]
#[proptest]
fn test_resolver_random_specifiers(
    #[strategy("[a-zA-Z0-9_./@~-]{1, 100}")] specifier: String,
) {
...
fn test_max_depth_invariant() {
    use crate::config::{AnalyzerConfig, DEFAULT_MAX_DEPTH};
    ...
    assert!(DEFAULT_MAX_DEPTH > 0, "Default max_depth should be positive");
    assert!(DEFAULT_MAX_DEPTH < 100_000, "Default max_depth should be reasonable");
}

/// Property test: max_modules enforcement invariant.
#[test]
fn test_max_modules_invariant() {
    use crate::config::{AnalyzerConfig, DEFAULT_MAX_MODULES};
    ...
    // Verify default is reasonable
    assert!(DEFAULT_MAX_MODULES > 0, "Default max_modules should be positive");
    assert!(DEFAULT_MAX_MODULES < 10_000_000, "Default max_modules should be reasonable");
}
...
fn test_path_alias_formats(
    ...
    use crate::resolver::aliases::resolve_path_alias;
    use std::path::Path;
