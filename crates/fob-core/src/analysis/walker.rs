...
use crate::graph::module_collection_plugin::{
    CollectedExport, CollectedImport, CollectedModule, CollectionState,
};
...
impl GraphWalker {
    ...
    pub async fn walk(
        ...
        while let Some((current_path, depth)) = queue.pop_front() {
            ...
            for export in &exports_clone {
                if let crate::graph::module_collection_plugin::CollectedExport::All { source } = export {
    ...
    ///
    /// Uses the existing parse_module_structure function from module_collection_plugin.
    fn parse_module(
        &self,
        code: &str,
    ) -> (Vec<CollectedImport>, Vec<CollectedExport>, bool) {
        crate::graph::module_collection_plugin::parse_module_structure(code)
    }
