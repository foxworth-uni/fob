...
///
/// ```rust,no_run
/// use fob_core::analysis::analyzer::Analyzer;
///
...
impl Analyzer {
    ...
    pub async fn analyze(self) -> Result<AnalysisResult> {
        ...
        let collection = walker.walk(runtime.clone()).await.map_err(|e| {
            crate::Error::Bundler(format!("Graph walker failed: {}", e))
        })?;

        // Build module graph from collected data
        let graph = ModuleGraph::from_collected_data(collection)
            .await
            .map_err(|e| crate::Error::Bundler(format!("Failed to build module graph: {}", e)))?;
