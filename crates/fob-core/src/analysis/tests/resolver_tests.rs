...
use crate::analysis::types::{AnalyzerConfig, ResolveResult};
...
async fn test_resolve_with_tsx_extension() {
    ...
    let result = resolver.resolve("./Component", &from, &runtime).await.unwrap();
...
async fn test_resolve_index_file() {
    ...
    let result = resolver.resolve("./components", &from, &runtime).await.unwrap();
...
async fn test_resolve_file_already_has_extension() {
    ...
    let result = resolver.resolve("./utils.ts", &from, &runtime).await.unwrap();
...
async fn test_path_alias_resolution() {
    ...
    let result = resolver.resolve("@/components/Button", &from, &runtime).await.unwrap();
...
async fn test_multiple_path_aliases() {
    ...
    config.cwd = Some(root.clone());
    config.path_aliases.insert("@".to_string(), "./src".to_string());
    config.path_aliases.insert("~".to_string(), "./lib".to_string());
    
    let resolver = ModuleResolver::new(config);
    let runtime = TestRuntime::new(root.clone());
    let from = root.join("src/index.ts");
    
    let result1 = resolver.resolve("@/components/Button", &from, &runtime).await.unwrap();
...
async fn test_external_prefix_match() {
    ...
    let result = resolver.resolve("react-dom", &from, &runtime).await.unwrap();
...
async fn test_scoped_package_is_external() {
    ...
    let result = resolver.resolve("@types/node", &from, &runtime).await.unwrap();
...
async fn test_unresolved_file_returns_unresolved() {
    ...
    let result = resolver.resolve("./nonexistent", &from, &runtime).await.unwrap();
...
async fn test_resolve_absolute_path() {
    ...
    let result = resolver.resolve(absolute_path.to_str().unwrap(), &from, &runtime).await.unwrap();
