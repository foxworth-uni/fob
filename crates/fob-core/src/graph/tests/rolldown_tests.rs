...
async fn test_module_collection_plugin_integration() {
    ...
    assert!(module_count >= 2, "Expected at least 2 modules, got {}", module_count);

    // Verify entry points were captured
    let entry_points = graph.entry_points().await.expect("get entry points");
    assert!(!entry_points.is_empty(), "Expected at least one entry point");
    ...
    assert!(
        unused_exports.iter().any(|item| {
            ...
            item.module_id.as_path().to_string_lossy().contains("utils.js")
        }),
