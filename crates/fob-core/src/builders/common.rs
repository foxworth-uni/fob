...
use crate::analysis::{
    stats::compute_stats, AnalysisResult, AnalyzedBundle, CacheAnalysis, TransformationTrace,
};
use crate::builders::{asset_registry::AssetRegistry, asset_plugin::AssetDetectionPlugin};
use crate::graph::module_collection_plugin::ModuleCollectionPlugin;
...
pub(crate) async fn execute_bundle(plan: BundlePlan) -> Result<AnalyzedBundle> {
    ...
    let graph = crate::graph::ModuleGraph::from_collected_data(collection_data)
        .await
